...
| 工具 | 作用 |
| --- | --- |
| `history_session_bootstrap` | 新对话开始时初始化或恢复项目会话；首次使用创建历史文件，后续读取全部历史摘要和最近一次完整交接 |
| `history_session_checkpoint` | 每轮任务完成后保存结构化进度，包括目标、发现、决策、修改文件、测试结果、遗留问题和下一步 |
...
历史文件使用可读的 Markdown 格式，可以随项目备份或纳入 Git，也方便开发者直接审阅和修订。检查点采用幂等写入，并要求在返回 `ok=true` 后才确认保存成功。
