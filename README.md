...
工作区页面现在会持续显示 MCP 和 Actions 的运行状态，并提供几个适合长期运行的控制项：

- **自动启动**：应用启动后恢复上次明确启用的服务。
- **自动恢复**：端口消失或进程异常时，由本地 watchdog 按退避间隔尝试恢复；手动停止的服务不会被重新拉起。
- **重新连接所有**：一次重启当前运行或异常的 MCP/Actions、重连已配置隧道，并立即执行健康检查。
- **健康检查自动刷新**：本地 endpoint、公网 `/mcp` 和 OAuth 元数据会周期检查，失败项会在界面和通知中标出。

第一次部署后，建议在工作区的“运行策略”中打开 MCP 与 Actions 的自动启动/自动恢复；如果只想临时停用某个服务，使用“停止”即可，应用会记住这个手动意图。
